...
export default function Home() {
  ...
  return (
    <div className="relative bg-black text-white overflow-hidden bg-grid-pattern min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
            {stats.map((stat, idx) => {
              ...
              return (
                <div key={idx} className="text-center p-4 bg-white/80 rounded-2xl border border-slate-200/80 shadow-sm shadow-slate-100/50">
            ...
            <div key={svc.id} className="glass-card p-8 rounded-2xl flex flex-col justify-between relative group overflow-hidden">
              ...
              <div>
                <div className="flex flex-wrap gap-2 mb-6">
                  {svc.tech.map((t, i) => (
                    <span key={i} className="text-[10px] bg-white/5 border border-white/10 px-2.5 py-1 rounded text-slate-700 font-mono">
                      {t}
                    </span>
                  ))}
                </div>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative z-10 bg-white/40 rounded-3xl border border-slate-200/60 shadow-sm shadow-slate-100/30">
        <div className="text-center max-w-3xl mx-auto mb-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {featuredCourses.map((course) => (
            <div key={course.id} className="bg-white border border-slate-200 shadow-sm hover:border-brand-purple/40 p-6 rounded-2xl flex flex-col justify-between transition-all hover:scale-[1.01] glass-card">
              <div>
                ...
                <h3 className="text-base sm:text-lg font-bold text-slate-800 mb-2 line-clamp-2 h-12 leading-tight">{course.title}</h3>
                <p className="text-xs text-slate-500 mb-6 line-clamp-3 leading-relaxed">{course.overview}</p>
              </div>
      ...
      <section id="contact-forms" className="py-24 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto relative z-10 border-t border-slate-200">
        ...
        <div className="flex justify-center space-x-4 mb-8">
          <button onClick={() => { setActiveTab('agency'); setSubmitStatus(null); }} className={`px-6 py-2.5 rounded-full font-bold text-xs uppercase tracking-widest transition-all ${activeTab === 'agency' ? 'bg-brand-purple text-black font-semibold' : 'bg-white/5 border border-white/10 text-slate-700 hover:text-white'}`}>
            Hire Our Agency
          </button>
          <button onClick={() => { setActiveTab('academy'); setSubmitStatus(null); }} className={`px-6 py-2.5 rounded-full font-bold text-xs uppercase tracking-widest transition-all ${activeTab === 'academy' ? 'bg-brand-purple text-black font-semibold' : 'bg-white/5 border border-white/10 text-slate-700 hover:text-white'}`}>
        ...
        <div className="space-y-4">
          {homeFaqs.map((faq, idx) => (
            <div key={idx} className="border border-white/10 rounded-2xl bg-white/5 overflow-hidden">
              ...
              {faqOpen[idx] && (
                <div className="px-6 pb-6 text-xs sm:text-sm text-slate-500 leading-relaxed border-t border-white/5 pt-4">
                  {faq.a}
                </div>
              )}
            </div>
