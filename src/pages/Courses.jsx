      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10 relative">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12 bg-slate-100 border border-slate-200 p-6 rounded-2xl">
          ...
          <div className="flex flex-wrap gap-2.5 max-w-full">
            {categories.map((cat, idx) => (
              <button key={idx} onClick={() => handleCategorySelect(cat)} className={`px-4 py-2 rounded-xl font-bold text-xs uppercase tracking-wider transition-all ${selectedCat === cat ? 'bg-brand-purple text-black' : 'bg-white border border-slate-200/80 shadow-sm border border-slate-200 text-slate-700 hover:text-slate-900'}`}>
                {cat}
              </button>
            ))}
          </div>
          ...
          <div className="relative w-full md:w-80">
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white border border-slate-200/80 shadow-sm border border-slate-250 rounded-xl pl-10 pr-4 py-3 text-xs text-slate-800 focus:border-brand-purple outline-none shadow-sm focus:border-brand-purple outline-none" placeholder="Search courses..." />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredCourses.map((c) => (
            <div key={c.id} className="glass-card p-6 rounded-2xl flex flex-col justify-between border border-slate-200/60 hover:border-brand-purple/20 transition-all hover:scale-[1.01]">
              <div>
                ...
                <p className="text-xs text-slate-500 leading-relaxed mb-6 line-clamp-3">{c.overview}</p>
              </div>
